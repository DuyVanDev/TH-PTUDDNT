...
const Home = ({navigation}) => {
  ...
  const initial = async () => {
    const admin = {
      userName: 'chuhaist123@gmail.com',
      password: 'duynguyen',
      point: '200',
      address: 'BinhDuong',
      role: 'user',
    ...
    await Users.doc(admin.userName).onSnapshot(u => {
      if (!u.exists) {
        auth()
          .createUserWithEmailAndPassword(admin.userName, admin.password)
          .then(() => {
            Users.doc(admin.userName).set({
              ...admin,
            });
          });
      }
  ...
  const categoriesData = [
    {
      ...
      books: [bookVanHoc, bookKienTruc, bookKinhTe],
    },
    {
      id: 2,
  ...
  useEffect(() => {
    initial();
    Users.doc('chuhaist123@gmail.com').onSnapshot(u => setProfile(u.data()));
    Books.onSnapshot(listbook => {
      const result = [];
      listbook.forEach(b => result.push(b));
      setMyBooks(result);
    });

    Categories.get().then(lstCategories => {
      const result = [];
      lstCategories.forEach(c => result.push(c));
    ...
    const renderItem = ({item, index}) => {
      return (
        <TouchableOpacity
          ...
          {/* Book Cover */}
          <Image
            source={{uri: item._data.bookCover}}
            resizeMode="cover"
            style={{
              width: 180,
              height: 250,
    ...
    const renderItem = ({item}) => {
      return (
        <TouchableOpacity
          ...
          onPress={() => setSelectedCategory(item._data.id)}>
          {selectedCategory == item._data.id && (
            <Text style={{...FONTS.h2, color: COLORS.white}}>
  ...
  function renderCategoryData(booksData) {
    var books = [];
    ...
    if (selectedCategoryBooks.length > 0) {
      selectedCategoryBooks[0].books.forEach(book => {
        books.push(booksData.filter(b => b.id == book.id)[0]);
      });
    }
          ...
          <TouchableOpacity
            ...
            <View style={{flex: 1, marginLeft: SIZES.radius}}>
              ...
              <View style={{flexDirection: 'row', marginTop: SIZES.base}}>
                {item?._data?.genre.includes('Adventure') && (
                  <View
                    style={{
                      justifyContent: 'center',
                      alignItems: 'center',
                ...
                {item?._data?.genre.includes('Romance') && (
                  <View
                    style={{
                      justifyContent: 'center',
                      alignItems: 'center',
                ...
                {item?._data?.genre.includes('Drama') && (
                  <View
                    style={{
                      justifyContent: 'center',
                      alignItems: 'center',
